<template>
  <div class="list-page list-page-files">
    <div class="header">
      <div class="title n-ellipsis">
        <n-breadcrumb separator=">">
          ...
          <n-breadcrumb-item v-for="item in parentInfo" :key="item.id" >
    ...
    <n-scrollbar style="max-height: calc(100vh - 190px);" @scroll="scrollHandle">
      <n-data-table v-model:checked-row-keys="checkedRowKeys" :row-key="row => row.id" :data="filesList" size="small"
                    :columns="columns" :bordered="false"></n-data-table>
    ...
    <n-modal v-model:show="showUserMenu">
      <n-card style="width: 600px;" title="自定义菜单">
        <template #header>
          自定义菜单 <a href="https://www.tjsky.net/?p=220#i-8" target="_blank">
      ...
      return h('div', {
        ...
        onClick: () => {
          if (row.kind === 'drive#folder') {
            router.push((route.path  + '/' + row.id + '/').replace('//','/'))
...
const getFileList = () => {
  ...
  let parent_id = route.name !== 'list' ? '*' : route.params.id[route.params.id.length - 1]
...
const initPage = () => {
  ...
  if (route.params.id && route.params.id !== '*') {
    let path = '';
    for (let i = 0; i < route.params.id.length; i++) {
      getFile(String(route.params.id[i]))
        .then(res => {
          path = path + '/' + res.data.id
          res.data.path = path
          parentInfo.value.push(res.data)
        })
    }
  } else {
    parentInfo.value = []
...
const getFile = (id: string) => {
  return http.get<FileInfo>('https://api-drive.mypikpak.com/drive/v1/files/' + id, {
    params: {
      _magic: '2021',
      thumbnail_size: 'SIZE_LARGE'
    }
